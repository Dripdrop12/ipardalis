...
date: 2025-04-27
description: "Learn why iPardalis, a small-batch panther chameleon breeding company, donates to Wildlife Madagascar, Responsible Reptile Keeping, Chameleon Forums, Reptifiles, USARK, and produces educational content for the Chameleon Academy."
home: true
execute:
  enabled: true
...
wm_start <- "20240501"
wm_months <- interval(ymd(wm_start), ymd(today())) %/% months(1)
wm_total <- wm_months * 27

rr_start <- "20240101"
rr_months <- interval(ymd(rr_start), ymd(today())) %/% months(1)
rr_total <- rr_months * 13

cf_start <- "20201201"
cf_years <- interval(ymd(cf_start), ymd(today())) %/% years(1)
cf_total <- cf_years * 250

rf_start <- "20240201"
rf_months <- interval(ymd(rf_start), ymd(today())) %/% months(1)
rf_total <- rf_months * 250

usark_start <- "20220201"
usark_years <- interval(ymd(usark_start), ymd(today())) %/% years(1)
usark_total <- usark_years * 250

total_donations <- wm_total + rr_total + cf_total + rf_total + usark_total
```

 ## Running total to all causes: **$`{r} total_donations`**
...
**Donation total:** $`{r} usark_total` 

---

### Sharing Knowledge: The Chameleon Academy & Reptiles Magazine
